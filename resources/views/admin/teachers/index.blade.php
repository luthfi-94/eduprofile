    ...
    <div class="card border-0 shadow-sm">
        <div class="card-header bg-white border-0 d-flex flex-column flex-md-row justify-content-between align-items-md-center gap-2">
            <div>
                ...
                <p class="text-muted mb-0">Manage teacher data with search and pagination.</p>
            </div>
        ...
        <div class="card-body">
            @if ($teachers->isEmpty())
                <div class="text-center py-5 text-muted">No teachers found.</div>
            @else
                        ...
                        <tbody>
                            @foreach ($teachers as $teacher)
                                <tr>
                                    <td>
                                        @if ($teacher->photo)
                                            <img src="{{ Storage::disk('public')->url($teacher->photo) }}" alt="{{ $teacher->name }}" class="rounded-circle" style="width: 48px; height: 48px; object-fit: cover;">
                                        @else
